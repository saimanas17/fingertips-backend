    ...
    steps {
        script {
            ...
            if (serviceExists.toInteger() > 1) {
                // Service already exists, update it to new code and remove old containers
                sh "docker service update --image ${ECR_REPO}:fingertips-backend-${BUILD_NUMBER} --detach ${SERVICE_NAME}"
            } else {
